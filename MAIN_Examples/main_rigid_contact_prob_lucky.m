...
%% For contact model
rigid_left=[0.29, geoheight];
rigid_right=[0.51, geoheight];
...
max_newton_iter = 100; tol = 5e-3; tol2 = 1e-7;
...
for step = 1:length(watch.dt)
    ...
    for iter = 1:max_newton_iter
        ...
        [K0, RHS, stress_new, SDV_new] = assemble_system(nod_adjele, area_nod, area_T3,...
        all_sd_set_node, all_sd_B, all_sd_E, ...
...
hold on
plot(seg(:,1),seg(:,2),'k-o','linewidth',2);