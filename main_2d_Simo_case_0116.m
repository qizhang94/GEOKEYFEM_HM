% The script simulates the Section 6.2 problem in Simo JC and Meschke G
% A new class of algorithms for classical plasticity extended to finite strains. Application to geomaterials
% Computational Mechanics (1993) 11, p253-p278
...
keypoint = [0, 0;
    ...
    foot, 0;
    0, 0];
meshsize = [8.88238/34, 8.88238/34];
[gcoord, ele_nods] = meshfooting('Uniform mesh', keypoint, meshsize, true); % This function will add more paths to the program
...
% MPa (E = 30000 psi)
K = 206.84272/3/(1-2*0.3); nu = 0.3; tau = 2; k_perm = 1e-14; mu_f = 1e-9; rho_buo = 0; % buoyant density of the mixture
data_const.tauG = tau/(3*K*(1 - 2*nu)/1/(1+nu));   % Stabilization multiplier, already divided by 2G
data_const.mobility = k_perm/mu_f;

phi = asin(42/sqrt(15037)); psi = phi; cohesion = 0.18919214*0.112/cos(phi)*sqrt(9+3*sin(phi)*sin(phi)); 
...
% Dirichlet BC
bcdof = [ndof*b_foot', ndof*b_foot'-1, ndof*b_left'-1, ndof*b_right'-1, ndof*b_bottom', ndof*b_bottom'-1, ndof*nnode + b_top'];
...
figure(1);
set(gcf, 'Color', 'w');
plot(-Total_U/num_steps*(0:1:num_steps), [0,-resi_force]/foot/1, 'b-d','Linewidth', 1, 'MarkerFaceColor','y'); grid on; % thickness in z direction: 1 unit
xlabel('Penetration depth (m)');
ylabel('Normalized Reaction Pressure', 'Interpreter', 'LaTeX');
