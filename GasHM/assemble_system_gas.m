...
% NS-FEM
function [K, RHS, stress_new, SDV_new] = assemble_system_gas(nod_adjele, area_nod, area_T3,...
...
for ino = 1:nnode
    ...
    % Constitutive model
    [stress_new(:, ino), SDV_new(:, ino), cto, ~] = AMCC_UMAT(Props(ino,:), stress(:, ino), strain_ino_new - strain_ino_old, SDV(:, ino));
...
RHS = [residual_1; residual_2];
    ...
    function [subSD_RHS, subSD_K] = cal_int_gascompress(x_coord, y_coord, area, x_subSD, y_subSD)
            ...
            for i = 1:4   % here "4" is not nnel, but the number of Gauss integration points
                ...
                % Comes from div(\dot{\vec{u}_s) by assuming constant sig_total
                % fs_coupling_coef = [1,1,0]*(material_data_const.Ce_t0\eye(3))*[1;1;0]; % Unit: 1/[Pressure]
                fs_coupling_coef = 0;
                Rigas = 8.3145*1E-6;
                Vmstp = 22.4*1E-3; % m^3/mol
                const_one = material_data_const.rhorock/Vmstp*material_data_const.V_L*material_data_const.P_L*Rigas*material_data_const.T;
                P_L = material_data_const.P_L; % For convenience
                
                % Evaluate (do not forget the POROSITY here!)
                subSD_RHS = subSD_RHS + weight(i) * ([N1; N2; N3]*(pgas_Gauss - pgas_Gauss_old)*(material_data_const.poro/K_f + fs_coupling_coef + ...
                    const_one/(P_L + pgas_Gauss)^2/pgas_Gauss) + ...
                    delta_t*[N1; N2; N3]*gradp_SD'*q_Darcy/K_f) * detJ;
                subSD_K = subSD_K + weight(i) * [N1; N2; N3]*(pgas_Gauss_old*material_data_const.poro/pgas_Gauss^2 + fs_coupling_coef + ...
                    pgas_Gauss_old/pgas_Gauss^2*const_one/(P_L + pgas_Gauss)^2  ...
                    +(-2)*(pgas_Gauss - pgas_Gauss_old)/pgas_Gauss*const_one/(P_L + pgas_Gauss)^3)*[N1, N2, N3] * detJ;
