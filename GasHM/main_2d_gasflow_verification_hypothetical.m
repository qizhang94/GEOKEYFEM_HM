...
% MPa
K = 2.8662e+03; nu = 0.15; tau = 2; k_perm = 1e-16; mu_f = 2e-11; rho_buo = 0; poro = 0.06; % buoyant density of the mixture
...
b_middle = find(gcoord(:,2)== 0);
temp = [gcoord(b_middle, 1), b_middle];
    temp = sortrows(temp);
    b_middle = temp(:, 2);
    x_middle = temp(:,1);
...
old_solution(nnode*ndof+1:end) = 2; % Initial uniform gas pressure of 2 [MPa]
old_solution(nnode*ndof+b_right) = 1; % Dirichelet boundary condition of p_gas
new_solution = old_solution;
traction_temp = sparse(ndof*nnode, 1);


% Dirichlet BC
bcdof = [ndof*b_left'-1, ndof*b_right'-1, ndof*b_top', ndof*b_bottom', nnode*ndof+b_left', nnode*ndof+b_right'];
...
watch.dt = 10*(4.^(1:1:15));   % time STEP INTERVAL
watch.now = 0; % start
...
for step = 1:length(watch.dt)
    ...
    for iter = 1:max_newton_iter
        ...
        [K, RHS, stress_new, SDV_new] = assemble_system_gas2(nod_adjele, area_nod, area_T3,...
        all_sd_set_node, all_sd_B, all_sd_E, ...
        new_solution, old_solution,...
...
% Pore pressure
figure
snscontour(node, element,0, u_xp,u_yp,p_nodal,'PP');
title('Pore pressure, MPa')
hold on

figure
plot(x_middle, new_solution(nnode*ndof + b_middle), 'b', 'LineWidth', 1); hold on; grid on;
plot(x_middle, sqrt(2.5 - 1.5*x_middle), 'r-.', 'LineWidth', 1); % Analytical
